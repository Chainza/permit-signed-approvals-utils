...
import {EthersProviderConnector} from './connector/ethers-provider.connector';
...
describe('Eip2612PermitUtils', () => {
    ...
    const connectors: Record<string, ProviderConnector> = {
        ethers: new EthersProviderConnector(
            wallet,
            AbiCoder.defaultAbiCoder(),
            Contract
        ),
        privateKey: new PrivateKeyProviderConnector(privateKey, web3),
    };

    Object.entries(connectors).forEach(([connectorName, connector]) => {
        describe(`${connectorName} connector`, () => {
            describe('disabledCheckSalt', () => {
                ...
                it('Build permit signature', async () => {
                    const signature =
                        await eip2612PermitUtils.buildPermitSignature(
                            permitParams,
                            chainId,
                            tokenName,
                            tokenAddress
                ...
                it('Build permit call data', async () => {
                    const callData =
                        await eip2612PermitUtils.buildPermitCallData(
                            ...
                            tokenName,
                            tokenAddress,
                            await eip2612PermitUtils.getTokenVersion(
                                tokenAddress
                            )
                        );
                ...
                it('Recover owner address from permit call data', async () => {
                    ...
                    const result =
                        await eip2612PermitUtils.recoverPermitOwnerFromCallData(
                            {
                                chainId,
                                tokenName,
                                tokenAddress,
                ...
                it('Recover owner address from DAI-like permit call data', async () => {
                    ...
                    const result =
                        await eip2612PermitUtils.recoverDaiLikePermitOwnerFromCallData(
                            {
                                chainId,
                ...
                it('Build DAI-like token permit signature', async () => {
                    const signature =
                        await eip2612PermitUtils.buildDaiLikePermitSignature(
                            daiPermitParams,
                            chainId,
                            tokenName,
                            tokenAddress
                ...
                it('getPermitTypeHash() should return token PERMIT_TYPEHASH', async () => {
                    const hash = await eip2612PermitUtils.getPermitTypeHash(
                        daiLikeTokenAddress
                    );
                ...
                it('Build permit signature for token with domain that does not include version', async () => {
                    ...
                    const domainTypeHash =
                        await eip2612PermitUtils.getDomainTypeHash(
                            tokenAddress
                        );
                ...
                it('Build permit signature for token with domain that does not include version', async () => {
                    const domainSeparator =
                        await eip2612PermitUtils.getDomainSeparator(
                ...
                it('syncRecoverPermitOwnerFromCallData()', async () => {
                    const owner =
                        eip2612PermitUtils.syncRecoverPermitOwnerFromCallData({
                            chainId: 1,
                            tokenName: '1INCH Token',
                            tokenAddress:
                                '0x111111111117dc0aa78b770fa6a738034120c302',
                            callData:
                                '0x000000000000000000000000fb3c7eb936caa12b5a884d612393969a557d4307' +
